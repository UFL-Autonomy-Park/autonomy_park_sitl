...
# Restore whatever nounset state the caller actually had before this,
# rather than forcing it back on - this file is meant to be *sourced* into
# an interactive shell (not just run as a script), and unconditionally
# leaving `set -u` on afterward broke bash's own tab-completion machinery
# in exactly that scenario ("COMPREPLY: unbound variable"), since
# bash-completion's internals aren't nounset-safe.
case $- in
    *u*) _sitl_env_had_nounset=1 ;;
    *)   _sitl_env_had_nounset=0 ;;
esac
set +u
source "$VENV_DIR/bin/activate"
(( _sitl_env_had_nounset )) && set -u
unset _sitl_env_had_nounset
