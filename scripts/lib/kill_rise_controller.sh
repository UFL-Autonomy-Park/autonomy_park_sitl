#!/usr/bin/env bash
# Kills the apark_rise_controller experiment launched by
...
# it (and the apark_rise_node it spawns as a child) in a new session whose
...
# the same as SIGINT and forwards a graceful shutdown to apark_rise_node,
# which lands the vehicle in its own `finally` block before exiting.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/process.sh"

kill_pidfile_group /tmp/rise_controller.pid "apark_rise_controller"
exit 0
